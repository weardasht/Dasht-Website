...
const PRODUCTS = [
  { id:'saqi',     word:'ساقی',      meaning:'Saqi',      originalPrice:2469, salePrice:1769, color:'White' },
  { id:'chainama', word:'چائے نامہ', meaning:'Chai-Nama', originalPrice:2469, salePrice:1769, color:'White' },
  { id:'sitare',   word:'ستارے',     meaning:'Sitare',    originalPrice:2469, salePrice:1769, color:'Black' },
  { id:'bazeecha', word:'بازیچہ',    meaning:'Bazeecha',  originalPrice:2469, salePrice:1769, color:'Black' },
];
const SIZES = ['S','M','L','XL'];

function discountPercent(p){ return Math.round((1 - p.salePrice / p.originalPrice) * 100); }
  ...
  grid.innerHTML = visible.map(p => {
    ...
    <div class="card">
      <a class="card-link" href="products/${p.id}.html">
        <span class="card-badge">${discountPercent(p)}% OFF</span>
        <div class="tee-art">
          <img src="assets/products/${p.id}/1.jpg" alt="${p.meaning}" style="width:100%;height:100%;object-fit:cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
          <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center;">${teeSVG(p)}</div>
        </div>
        <div class="card-tag">
          <span class="card-name">${p.meaning}</span>
          <span class="card-meaning">${p.color}</span>
        </div>
      </a>
